"""Train a ship-strike risk classifier with MLflow tracking.

Trains an XGBoost (and optionally LightGBM) binary classifier to
predict which H3 cells have historical ship–whale collisions.

Key design decisions:
  - Spatial block CV to prevent data leakage from nearby cells
  - scale_pos_weight to handle extreme class imbalance (67/1.8M)
  - AUC-PR as primary metric (not accuracy or AUC-ROC)
  - Optuna for Bayesian hyperparameter search
  - MLflow logs params, metrics, plots, and model artifact

Usage:
    uv run python pipeline/analysis/train_strike_model.py
...
from pipeline.analysis.extract_features import extract_strike_features
from pipeline.config import ML_DIR, MLFLOW_TRACKING_URI, STRIKE_FEATURES_FILE
...
# ── MLflow config ───────────────────────────────────────────
EXPERIMENT_NAME = "strike_risk"
ARTIFACTS_DIR = ML_DIR / "artifacts" / "strike"

# ── Feature columns ────────────────────────────────────────
# Columns that are NOT features (identifiers, targets, CV metadata)
NON_FEATURE_COLS = {
    "h3_cell",
    "cell_lat",
    "cell_lon",
...
def _get_feature_cols(df: pd.DataFrame) -> list[str]:
    """Return feature column names (everything not in NON_FEATURE_COLS)."""
...
def train_single_model(
    ...
    """Train a single XGBoost model with spatial CV evaluation.

    Returns (model trained on full data, cv_metrics dict).
    """
    ...
    if log_to_mlflow:
        ...
        plot_feature_importance(
            importance,
            top_n=25,
            title="Strike Risk — XGBoost Feature Importance",
            save_path=imp_path,
        )
...
def _optuna_objective(trial, df, feature_cols):
    """Optuna objective: maximize spatial CV average precision."""
    y = df[TARGET_COL]
    X = df[feature_cols]
    scale_pos_weight = _compute_scale_pos_weight(y)
...
def tune_hyperparameters(
    ...
    log.info(
        "Best trial: AP=%.4f  params=%s",
        study.best_value,
        study.best_params,
    )

    # Merge best trial params with defaults
    best_params = DEFAULT_PARAMS.copy()
    best_params.update(study.best_params)

    # Log Optuna study summary to MLflow
    mlflow.log_param("optuna_n_trials", n_trials)
    mlflow.log_param("optuna_best_ap", round(study.best_value, 4))
...
def main(tune: bool = False, n_trials: int = 50) -> None:
    """Run the full strike model training pipeline."""
    matplotlib.use("Agg")
    ...
    # Load features
    if STRIKE_FEATURES_FILE.exists():
        log.info("Loading cached features: %s", STRIKE_FEATURES_FILE)
        df = pd.read_parquet(STRIKE_FEATURES_FILE)
    else:
        log.info("Extracting features from PostGIS…")
        df = extract_strike_features()
        ML_DIR.mkdir(parents=True, exist_ok=True)
        df.to_parquet(STRIKE_FEATURES_FILE, index=False)

    log.info("Dataset: %d rows × %d cols", *df.shape)
    log.info(
        "Class balance: %d positive (%.4f%%)",
        df[TARGET_COL].sum(),
        100 * df[TARGET_COL].mean(),
    )

    with mlflow.start_run(run_name="strike_xgboost"):
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train strike risk classifier")
    parser.add_argument(
        "--tune",
        action="store_true",
